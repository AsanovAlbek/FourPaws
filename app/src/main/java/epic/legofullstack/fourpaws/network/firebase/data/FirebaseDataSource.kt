...
class FirebaseDataSource(private val firestore: FirebaseFirestore) {
    ...
    suspend fun getSheltersByArea(areaId: Int): List<ShelterPreviewDto> =
        firestore.collection(SHELTER_COLLECTION)
            .whereEqualTo("area.id", areaId)
            .get()
            .await()
            .toObjects(ShelterPreviewDto::class.java)

    suspend fun getPetsByArea(areaId: Int): List<PetPreviewDto> =
        firestore.collection(PET_COLLECTION)
            .whereEqualTo("areaId", areaId)
            .get()
            .await()
            .toObjects(PetPreviewDto::class.java)
